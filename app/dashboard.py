...
# ════════════════════════════════════════════════════════════════════════════
with tab_chat:
    # ── Chatbot tab wired to the shared text-to-pandas engine (app/chatbot.py) ──
    from app.chatbot import answer_question

    st.markdown(
        '<div class="ms-sh"><span class="ms-sh-title">Procurement AI Chatbot</span>'
        '<span class="ms-sh-pill">GPT-4o · text-to-pandas</span></div>',
        unsafe_allow_html=True,
    )
    st.caption(
        "Ask in plain English — the assistant writes pandas, **runs it** on the Gold "
        "tables, and answers with the real computed result. Sidebar filters apply."
    )

    @st.cache_data(show_spinner=False)
    def _load_chat_extra():
        extra = {}
        for fname, key in [
    ...
    SAMPLE_QUESTIONS = [
        ...
        "What's the average predicted award value?",
